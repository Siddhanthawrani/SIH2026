  ...
  return (
    <div className="grid gap-6 lg:grid-cols-[0.95fr_1.05fr]">
      <div className="nv-card p-6 md:p-8">
        <div className="flex items-center gap-2 text-[11px] font-bold tracking-[0.16em] text-slate-500"><SlidersHorizontal size={14} /> TUNE THE MODEL — LIVE DEMO</div>
        <h3 className="mt-2 text-2xl font-extrabold tracking-tight text-[#0a1628]">Drag the weights. Watch NERVE re-think.</h3>
        <p className="mt-2 text-sm leading-relaxed text-slate-600">Every segment across 8 states is scored on 14 fused signals. Adjust priorities — the ranking recomputes instantly, exactly as the control tower does.</p>
        <div className="mt-4 flex flex-wrap gap-2">
          {['Vaccines (cold-chain)', 'PDS Rice (12T)', 'BRO equipment'].map((c) => (
            <button key={c} onClick={() => setCargo(c)} className={`rounded-full px-3.5 py-1.5 text-[12px] font-semibold transition ${cargo === c ? 'bg-[#0a1628] text-white' : 'border border-slate-200 bg-white text-slate-600 hover:border-slate-300'}`}>{c}</button>
          ))}
        </div>
          ...
          {PARAMS.map((p) => (
            <div key={p.id}>
              <div className="flex items-center justify-between gap-3">
                <div>
                  <div className="text-[13.5px] font-bold text-[#0a1628]">{p.label}</div>
                  <div className="text-[11.5px] text-slate-500">{p.desc}</div>
                </div>
                <span className="rounded-lg border border-slate-200 bg-white px-2 py-1 font-mono text-[12px] font-bold text-[#0a1628] shadow-sm">{weights[p.id]}</span>
              </div>
      ...
      <div className="flex flex-col gap-4">
        <div className="nv-dark-stage rounded-[20px] p-6 md:p-8">
          <div className="flex items-center gap-2 text-[11px] font-bold tracking-[0.16em] text-slate-300"><BrainCircuit size={14} /> NERVE AI CORE • DECISION</div>
          <div className="mt-3 flex flex-wrap items-end justify-between gap-3">
            <div>
              <div className="text-[13px] font-semibold text-slate-400">BEST ROUTE FOR <span className="text-white">{cargo.toUpperCase()}</span></div>
              <div className="mt-1 text-3xl font-extrabold tracking-tight text-white md:text-4xl">{best.name}</div>
            </div>
            <div className="text-right">
              <div className="text-5xl font-extrabold tracking-tight text-emerald-300">{best.score}</div>
              <div className="text-[11px] font-bold tracking-widest text-slate-400">SAFETY SCORE / 100</div>
            </div>
          </div>
          <div className="mt-5 space-y-3">
            {result.map((r, i) => (
              <div key={r.name} className={`rounded-2xl border p-4 ${i === 0 ? 'border-emerald-300/30 bg-emerald-400/[0.08]' : 'border-white/10 bg-white/[0.04]'}`}>
                <div className="flex flex-wrap items-center justify-between gap-2">
                  ...
                  <span className="text-[12px] font-semibold text-slate-400">ETA {r.eta}</span>
                </div>
                <div className="mt-2.5 h-2 overflow-hidden rounded-full bg-white/10">
                  <div className={`h-full rounded-full transition-all duration-500 ${r.score > 75 ? 'bg-emerald-400' : r.score > 55 ? 'bg-amber-300' : 'bg-red-400'}`} style={{ width: `${Math.max(4, r.score)}%` }} />
                </div>
          ...
          <p className="mt-4 rounded-xl border border-white/10 bg-white/[0.04] p-3.5 text-[12.5px] leading-relaxed text-slate-300">
        ...
        <div className="grid grid-cols-3 gap-3">
          {[['14', 'live signals'], ['40ms', 'per segment'], ['8', 'states fused']].map(([v, l]) => (
            <div key={l} className="nv-card p-4 text-center">
              <div className="text-2xl font-extrabold tracking-tight text-[#0a1628]">{v}</div>
              <div className="text-[11px] font-semibold tracking-wider text-slate-500">{l.toUpperCase()}</div>
            </div>
          ))}
        </div>
      </div>
